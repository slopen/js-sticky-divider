...
(function($) {
	...
	var onScroll = function(e){
		...
		var headerOffset = scrollEl.opts.headerOffset;
		...
		$labelEl.each(function(indx, el){
			...
			var elOffsetT = el.offsetTop,			// offset from top of scrolling el
				...
				elH = $(el).outerHeight(),			// height of el
			...
			if( elOffsetTOrig > scrollT + (scrollDir == 'up' ? headerOffset : 0)){
			...
			if( nextEl ){
				...
				|| (scrollDir == 'up' && elOffsetT-headerOffset < scrollT)){
					return;
				}
			}

			if (scrollDir == 'up') {
				el.style.top = (elDiff + headerOffset) + 'px';
			} else {
